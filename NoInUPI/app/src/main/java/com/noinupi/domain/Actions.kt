...
object Actions {
    ...
    val SendUpi = Action(
        ...
        steps = listOf(
            ActionStep(
                match = Regex("bank.{0,30}(name|ifsc)|bank.?name|first 4 letters", RegexOption.IGNORE_CASE),
                reply = "IOB",
                label = "Selecting Indian Overseas Bank"
            ),
            ActionStep(
                match = Regex("select option.*indian overseas bank|\\b1\\.\\s*send money", RegexOption.IGNORE_CASE),
                reply = "1",
                label = "Selecting Send Money"
            ),
            ActionStep(
                match = Regex("send money to|1\\.\\s*mobile no|3\\.\\s*upi id|saved beneficiary|ifsc", RegexOption.IGNORE_CASE),
                reply = "3",
                label = "Selecting UPI ID"
            ),
            ActionStep(
                match = Regex("enter\\s+upi\\s+id", RegexOption.IGNORE_CASE),
                reply = "{vpa}",
                label = "Sending UPI ID"
            ),
            ActionStep(
                match = Regex("enter\\s+amount|amount\\s+in\\s+rs", RegexOption.IGNORE_CASE),
                reply = "{amount}",
                label = "Sending amount"
            ),
            ActionStep(
                match = Regex("enter\\s+a\\s+remark|remark.*skip", RegexOption.IGNORE_CASE),
                reply = "1",
                label = "Skipping remark"
            ),
            ActionStep(
                match = Regex("enter\\s+upi\\s+pin|upi\\s+pin\\s+to\\s+proceed", RegexOption.IGNORE_CASE),
                reply = null,
                label = "Enter UPI PIN manually"
            ),
            ActionStep(
                match = Regex("payment.*successful|is\\s+successful|successful.*ref", RegexOption.IGNORE_CASE),
                done = true,
    ...
    val CheckBalance = Action(
        ...
        steps = listOf(
            ActionStep(
                ...
                reply = null,
                label = "Enter UPI PIN in the carrier dialog"
            ),
            ActionStep(
